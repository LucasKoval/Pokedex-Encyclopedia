...
const DetailCard = props => {
    ...
    return (
        <div className="card p-2 pt-3 border-groove max-size bg-light">
                                    ...
                                    <div className="col-6 col-sm-3">
                                        <div className="fw-bold text-secondary">Type </div>
                                        {
                                            props.types.map((type, index) => {
                                                return (
                                                    <div key={index} className="btn my-1 me-1 border border-dark text-capitalize" style={{ backgroundColor: typeColors[type.type.name] }}>{type.type.name}</div>
                                                )
                                            })
                                        }
                                    </div>
                                    <div className="col-6 col-sm-5">
                                        <div className="fw-bold text-secondary">Abilities </div>
                                        {
                                            props.abilities.map((ability, index) => {
                                                return (
                                                    <div key={index} className="text-capitalize"><img className="w-25" src='/static/icons/attack-2.png' alt="Attack"/>{ability.ability.name}</div>
                                                )
                                            })
                                        }
                                    </div>                                    
